...
{
\textbf{}   
\\$\bullet$ Based on the closed collaboration with the product team, designed the solution and created from scratch the highly scalable RESTful service that allowed remote IoT devices to update their configurations (AWS Lambda, Terraform, Kotlin, http4k)
\\$\bullet$ Created the highly scalable cross-database replication service (DynamoDB, Lambda, Terraform, Python)
\\$\bullet$ Created and integrated into the GitLab CI the E2E testing pipeline that allowed to increase the deployment frequency
...
\\$\bullet$ Migrated legacy Bamboo CI pipelines to GitLab ones
 }
\job
{Nov. 2018 }{ Oct. 2022}
{Intellias}
